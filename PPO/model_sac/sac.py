...
class SAC(object):
    ...
    def select_action(self, state_list, evaluate=False):   # state_list = [frame_stack, goal, speed]
        ...
        frame_list = Variable(torch.from_numpy(frame_list)).float().cuda()   # 220707 처음에 주석처리되 있었음
        goal_list = Variable(torch.from_numpy(goal_list)).float().cuda()     # main_sac 실행시 argument가 --gpu true
