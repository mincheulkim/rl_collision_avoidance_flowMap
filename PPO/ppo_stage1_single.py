...
from model.net import MLPPolicy, CNNPolicy, LM_Policy, stacked_LM_Policy, concat_LM_Policy, depth_LM_Policy, baseline_LM_Policy, baseline_ours_LM_Policy, ours_LM_Policy
...
from model.ppo import generate_action, generate_action_human, generate_action_human_groups, generate_action_human_sf, generate_action_LM, generate_action_stacked_LM, generate_action_concat_LM, generate_action_depth_LM, generate_action_baseline_LM, generate_action_baseline_ours_LM, generate_action_ours_LM
...
policy_list = 'ours_LM'      # select policy. [LM, stacked_LM, ''(2018), concat_LM(convLSTM), depth_LM(TODO), baseline_LM(IROS2021)]
...
def run(comm, env, policy, policy_path, action_bound, optimizer):
    ...
    for id in range(MAX_EPISODES):
        ...
        elif policy_list == 'baseline_LM' or policy_list == 'ours_LM':
        ...
        while not terminal and not rospy.is_shutdown():
            ...
            if policy_list=='LM':  # LM: 60x60
                ...
                v, a, logprob, scaled_action, Sensor_map, LM_stack =generate_action_ours_LM(env=env, state_list=robot_state, pose_list=pose_list, velocity_list=speed_poly_list, policy=policy, action_bound=action_bound, LM_stack=LM_stack, index=labels, mode=test_policy)
            ...
            if global_step % HORIZON == 0:
                ...
                if policy_list=='LM':  # LM: 60x60    # 211214
                    ...
                    last_v_r, _, _, _, _, _ = generate_action_ours_LM(env=env, state_list=state_next_list_new, pose_list=pose_next_list, velocity_list=speed_poly_next_list, policy=policy, action_bound=action_bound, index=labels, LM_stack=LM_stack, mode=test_policy)
            ...
            if env.index == 0 and not (step == 1 and terminal) and not_update_policy is not True:
                ...
                # 220124                        
                    ...
                    if len(buff) > HORIZON - 1:
                        s_batch, goal_batch, speed_batch, a_batch, r_batch, d_batch, l_batch, v_batch, sensor_map_batch, local_maps_batch = \
                            transform_buffer_baseline_ours_LM(buff=buff)
                        ...
                        ppo_update_stage1_baseline_ours_LM(policy=policy, optimizer=optimizer, batch_size=BATCH_SIZE, memory=memory,
                                            epoch=EPOCH, coeff_entropy=COEFF_ENTROPY, clip_value=CLIP_VALUE, num_step=HORIZON,
                                            num_env=1, frames=LASER_HIST,
                                            obs_size=OBS_SIZE, act_size=ACT_SIZE)   # 211214
...
if __name__ == '__main__':
    ...
    if rank == 0:
        ...
        elif policy_list == 'ours_LM':   # 220124
            policy_path = 'policy'
            policy = ours_LM_Policy(frames=LASER_HIST, action_space=2)
